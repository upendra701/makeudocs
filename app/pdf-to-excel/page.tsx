...
const extractPageRows = (items: Array<{ str: string; transform: number[]; width?: number }>): Row[] => {
  const tokens: PdfToken[] = items.map((item) => ({
...
export default function PdfToExcelPage() {
  ...
  const chooseFile = (nextFile?: File) => {
    ...
    setFile(nextFile); setPreview([]); setError("");
  };
  ...
  const extract = async () => {
    if (!file) return;
    setIsExtracting(true); setProgress(0); setError("");
    try {
      ...
      for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber++) {
        ...
        const rows = extractPageRows(content.items.filter((item): item is typeof item & { str: string; transform: number[] } => "str" in item && "transform" in item));
  ...
  const reset = () => { setFile(null); setPreview([]); setProgress(0); setError(""); };
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_50%_0%,rgba(219,234,254,.72),transparent_34rem),linear-gradient(180deg,#f8fbff_0%,#f8fafc_45%,#fff_100%)] px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <section className="mx-auto max-w-3xl text-center">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-blue-100 bg-white/80 px-3 py-1 text-xs font-semibold text-blue-700 shadow-sm"><span className="h-1.5 w-1.5 rounded-full bg-blue-600" /> Browser-based PDF table extraction</div>
          <h1 className="text-4xl font-bold tracking-tight text-zinc-950 sm:text-5xl">PDF to Excel</h1>
          <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-zinc-600 sm:text-lg">Extract selectable PDF text and table-like columns into a real Excel workbook. Your PDF is processed in your browser.</p>
        </section>
        <section className="mx-auto mt-10 max-w-4xl rounded-3xl border border-zinc-200 bg-white p-5 shadow-[0_24px_70px_rgba(15,23,42,.10)] sm:p-8">
          {!file ? (
            <div onDragOver={(event) => event.preventDefault()} onDrop={handleDrop} className="rounded-2xl border-2 border-dashed border-blue-200 bg-blue-50/50 px-6 py-14 text-center transition hover:border-blue-400 hover:bg-blue-50">
              ...
              <label className="mt-6 inline-flex cursor-pointer items-center rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-200 transition hover:bg-blue-700">Choose PDF<input type="file" accept="application/pdf,.pdf" onChange={handleInput} className="hidden" /></label>
            </div>
          ) : (
            <div>
              <div className="flex flex-col gap-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate font-semibold text-zinc-900">{file.name}</p><p className="mt-1 text-xs text-zinc-500">{(file.size / 1024 / 1024).toFixed(2)} MB</p></div><button onClick={reset} className="rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-100">Choose another</button></div>
              {isExtracting && <div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50 p-4"><div className="flex items-center justify-between text-sm font-medium text-blue-900"><span>Extracting PDF tables…</span><span>{progress}%</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-blue-100"><div className="h-full rounded-full bg-blue-600 transition-all" style={{ width: `${progress}%` }} /></div></div>}
              {!preview.length && !isExtracting && <button onClick={extract} className="mt-5 w-full rounded-xl bg-blue-600 px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-blue-200 transition hover:bg-blue-700">Extract tables &amp; preview</button>}
              {preview.length > 0 && <div className="mt-7"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-zinc-900">Extraction preview</p><p className="mt-1 text-xs text-zinc-500">{preview.length} page{preview.length === 1 ? "" : "s"} · {totalRows} rows</p></div><button onClick={exportExcel} disabled={isExporting} className="rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-100 transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60">{isExporting ? "Preparing Excel…" : "Download Excel"}</button></div><div className="mt-5 space-y-5">{preview.map((page) => <div key={page.page} className="overflow-hidden rounded-2xl border border-zinc-200 bg-white"><div className="border-b border-zinc-200 bg-zinc-50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-zinc-500">Page {page.page}</div><div className="max-h-[420px] overflow-auto"><table className="min-w-full border-collapse text-left text-sm"><tbody>{page.rows.slice(0, 100).map((row, rowIndex) => <tr key={rowIndex} className="border-b border-zinc-100 last:border-0">{row.map((cell, cellIndex) => <td key={cellIndex} className="whitespace-nowrap border-r border-zinc-100 px-3 py-2.5 align-top text-zinc-700 last:border-0">{cell}</td>)}</tr>)}</tbody></table></div>{page.rows.length > 100 && <p className="border-t border-zinc-100 px-4 py-2 text-xs text-zinc-400">Preview shows the first 100 rows. The Excel export contains all extracted rows.</p>}</div>)}</div><button onClick={extract} className="mt-5 text-sm font-semibold text-blue-600 hover:text-blue-700">Re-extract PDF</button></div>}
            </div>
          )}
          {error && <div className="mt-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm leading-6 text-red-700">{error}</div>}
        </section>
        <section className="mx-auto mt-8 grid max-w-4xl gap-4 sm:grid-cols-3">{[["Real Excel file","Export editable .xlsx cells instead of a PDF image."],["Browser processing","The PDF content is extracted locally in your browser."],["Table preview","Review extracted rows before downloading the workbook."]].map(([title, text]) => <div key={title} className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm"><p className="font-semibold text-zinc-900">{title}</p><p className="mt-2 text-sm leading-6 text-zinc-500">{text}</p></div>)}</section>
        <p className="mx-auto mt-6 max-w-4xl text-center text-xs leading-5 text-zinc-400">Best results come from PDFs containing selectable text with consistent table columns. Scanned/image-only PDFs are not OCR-processed by this tool yet.</p>
      </div>
    </main>
  );
}
